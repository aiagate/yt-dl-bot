...
class ArtifactDiscoveryError(Exception):
    def __init__(self, missing, candidates):
...
def discover_download_artifacts(
    *,
    ...
    existing_metadata = tuple(
        path for path in metadata if path_exists(path)
    )
    existing_thumbnails = tuple(
        path for path in thumbnails if path_exists(path)
    )
