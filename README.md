...
└── cache/                 # ダウンロード要求ごとの専用ディレクトリと一時グラフ
logs/
...
動画ダウンロードは要求ごとに `TMP_PATH` 内の専用ディレクトリを使います。同じ動画を
同時に要求しても一時ファイルを共有しません。同一 bot プロセス内では、保存先の衝突検査・
移動・失敗時のロールバックをまとめて直列化します。同じ分の同じ動画など、保存先が重なる
要求では先に保存した成果物を保持し、後の保存はエラーとして通知します。
複数の bot プロセスから同じ保存先へ書き込む運用には対応していません。

成功したダウンロードの専用一時ディレクトリは削除します。失敗・キャンセル時の一時ファイルは
調査や復旧のため残すので、不要になったものは運用者が削除してください。
